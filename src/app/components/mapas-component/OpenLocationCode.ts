...
/**
 * https://github.com/google/open-location-code/tree/main/typescript
...
export default class OpenLocationCode {
    ...
    public static encode(
        ...
        let latVal = Math.floor(
            Math.round(
                (editedLatitude + OpenLocationCode.LATITUDE_MAX_) * OpenLocationCode.FINAL_LAT_PRECISION_ * 1e6
            ) / 1e6
        );
        let lngVal = Math.floor(
            Math.round(
                (editedLongitude + OpenLocationCode.LONGITUDE_MAX_) * OpenLocationCode.FINAL_LNG_PRECISION_ * 1e6
            ) / 1e6
        );
    ...
    public static recoverNearest(shortCode: string, latitude: number, longitude: number): string {
        ...
        const paddingLength =
            OpenLocationCode.SEPARATOR_POSITION_ - shortCodeUpper.indexOf(OpenLocationCode.SEPARATOR_);
